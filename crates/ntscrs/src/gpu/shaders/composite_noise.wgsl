...
// 1D Simplex noise based on Ashima Arts (GLSL)
...
fn simplex_1d(x: f32, seed: f32) -> f32 {
    ...
    var i = vec2<f32>(i0, i1);
    var f = vec2<f32>(x0, x1);
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Use a PCG state initialized per row to pick an offset and noise seed
    // To mimic the CPU: each row has a different offset and seed derived from global seed + row index
    var rng_state = pcg_hash(params.seed + row_idx);
    let noise_seed = f32(rng_state) / f32(0xffffffffu) * 289.0; // simple hash to f32 seed
    rng_state = pcg_hash(rng_state);
    let offset = rand_f32(&rng_state) * f32(width);

    let x = (f32(col_idx) + offset) * params.composite_noise_frequency;
    let noise = fbm_1d(x, params.composite_noise_detail, noise_seed);

    y_plane[index] += noise * params.composite_noise_intensity * 0.25;
}
